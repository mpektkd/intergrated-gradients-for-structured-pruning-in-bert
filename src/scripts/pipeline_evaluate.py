...
def load_configuration(model_name_or_path=None, task_name=None, max_seq_length=None):
    # Disable logging
    logger = logging.getLogger("pipeline_evaluate")
...
def pipe():
    ...
    for task in npy_data:
        ...
        for mask_key, mask_value in npy_data[task][f'balanced_samples_{_samples}'].items():
            ...
            head_mask = torch.from_numpy(mask_value).to(device)

            preds, labels = inference(device, m_config, t_config, head_mask)
            ...
            logger.info(f"Task {task}, head_mask {extract_number(mask_key)}:")
